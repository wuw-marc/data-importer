...
pimcore.registerNS("pimcore.plugin.pimcoreDataImporterBundle.configuration.components.mapping.datatarget.relations");
pimcore.plugin.pimcoreDataImporterBundle.configuration.components.mapping.datatarget.relations = Class.create(pimcore.plugin.pimcoreDataImporterBundle.configuration.components.abstractOptionType, {
    ...
    initAttributeStore: function (attributeStore) {
        ...
        const classIdForCache = this.dataObjectClassId + 'advanced_relations';
        ...
        if (targetFieldCache[classIdForCache] && targetFieldCache[classIdForCache][transformationResultType]) {

            if (targetFieldCache[classIdForCache][transformationResultType].loading) {
                setTimeout(this.initAttributeStore.bind(this, attributeStore), 400);
            } else {
                attributeStore.loadData(targetFieldCache[classIdForCache][transformationResultType].data);
            }


        } else {
            targetFieldCache = targetFieldCache || {};
            targetFieldCache[classIdForCache] = targetFieldCache[classIdForCache] || {};
            targetFieldCache[classIdForCache][transformationResultType] = {
                loading: true,
                data: null
            };
            ...
            Ext.Ajax.request({
                ...
                params: {
                    ...
                    'system_write': 1,
                    'advanced_relations': true
                },
                success: function (response) {
                    let data = Ext.decode(response.responseText);

                    targetFieldCache[classIdForCache][transformationResultType].loading = false;
                    targetFieldCache[classIdForCache][transformationResultType].data = data.attributes;
